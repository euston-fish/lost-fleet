...
function Stats(norm_stats) {
  ...
  this.Construct = {
    Rn: leng(norm_stats.Construct.Rn.map((v) => 10*norm_scaled_value(v))),
  ...
  this.Misc = {
    ...
    Cp: norm_stats.Misc.Cp.map((v) => 100*norm_scaled_value(v)),
    Tr: leng(norm_stats.Misc.Cp.map((v) => 70*norm_scaled_value(v)))
  }
  this.cost = this.norm_stats.values().map((group) =>
